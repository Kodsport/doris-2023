...
void dfs(int u, p2 rem, vi& vis, vvi& edges)
{
    if (vis[u]) return;
    vis[u] = 1;
    repe(e, edges[u]) if(p2(u,e)!=rem&&p2(e,u)!=rem) dfs(e, rem, vis, edges);
}

int countcomponents(p2 rem, vvi& edges)
{
    int cnt = 0;
    vi vis(edges.size());
    rep(i, edges.size())
...
void findhead(int u, vi& vis, vvi& edges)
{
    if (vis[u]) return;
    vis[u] = 1;
    repe(e, edges[u]) findhead(e,vis,edges);
}
...
int32_t main()
{
    ...
    int cc = countcomponents(p2(-1, -1), edges);
    vp2 unused;
    repe(edge, edgeslist)
    {
        int nc = countcomponents(edge, edges);
        if (nc==cc)
        {
            unused.emplace_back(edge);
        }
    }
    ...
    rep(i, cc - 1)
    {
        cout << unused[i].first + 1 << " " << unused[i].second + 1 << " " << comphead[i] + 1 << " " << comphead[i + 1] + 1 << "\n";
    } 
